...
-- is: connect as analytics_app, then `SET LOCAL ROLE <work role>` for the work at
...
-- band and rewires ANALYTICS_DATABASE_URL. Until then the platform keeps
-- connecting as `andpay` exactly as before.
--
-- Membership is EXACTLY this context's work roles and nothing else. That is the
-- C4 boundary expressed in the database: analytics_app cannot enter another
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT FROM pg_roles WHERE rolname = 'analytics_app') THEN
    CREATE ROLE analytics_app LOGIN NOSUPERUSER NOBYPASSRLS NOINHERIT NOCREATEDB NOCREATEROLE;
  END IF;
END $$;
...
GRANT analytics_write, analytics_read, analytics_relay TO analytics_app;
