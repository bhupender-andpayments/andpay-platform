...
-- is: connect as orchestrator_app, then `SET LOCAL ROLE <work role>` for the work at
...
-- band and rewires ORCHESTRATOR_DATABASE_URL. Until then the platform keeps
-- connecting as `andpay` exactly as before.
--
-- Membership is EXACTLY this context's work roles and nothing else. That is the
-- C4 boundary expressed in the database: orchestrator_app cannot enter another
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT FROM pg_roles WHERE rolname = 'orchestrator_app') THEN
    CREATE ROLE orchestrator_app LOGIN NOSUPERUSER NOBYPASSRLS NOINHERIT NOCREATEDB NOCREATEROLE;
  END IF;
END $$;
...
GRANT orchestrator_write TO orchestrator_app;
