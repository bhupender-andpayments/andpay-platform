...
-- is: connect as identity_app, then `SET LOCAL ROLE <work role>` for the work at
...
-- band and rewires IDENTITY_DATABASE_URL. Until then the platform keeps
-- connecting as `andpay` exactly as before.
--
-- Membership is EXACTLY this context's work roles and nothing else. That is the
-- C4 boundary expressed in the database: identity_app cannot enter another
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT FROM pg_roles WHERE rolname = 'identity_app') THEN
    CREATE ROLE identity_app LOGIN NOSUPERUSER NOBYPASSRLS NOINHERIT NOCREATEDB NOCREATEROLE;
  END IF;
END $$;
...
GRANT identity_write, identity_read, identity_relay TO identity_app;
