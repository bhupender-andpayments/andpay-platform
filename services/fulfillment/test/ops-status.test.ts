...
async function seedShipment(status: string, awb = `AWB-${randomUUID()}`): Promise<{ shptId: string; programId: string; awb: string }> {
  ...
  `
  return { shptId: shptUuid, programId: programUuid, awb }
}
...
describe('ops status correction and privileged terminal override (spec 10c Task 6)', () => {
  ...
  it('correctStatus replay (same clientKey) is deduped and does not double-apply', async () => {
    ...
    const events = await db.$queryRaw<{ n: bigint }[]>`SELECT count(*) AS n FROM shpt_status_event WHERE shpt_id = ${seeded.shptId}::uuid`
  ...
  it('overrideTerminal rejects an empty or whitespace-only reason', async () => {
    ...
    await expect(overrideTerminal(db, {
      shptId: seeded.shptId, status: 'IN_TRANSIT', courierTimestamp: new Date(),
    ...
    expect((await shptRow(seeded.shptId)).status).toBe('DELIVERED')
  })
  ...
  it('overrideTerminal with a real reason reopens a DELIVERED shpt, audits the reason on the domain row, and emits a fact WITHOUT the reason text', async () => {
    ...
    expect(f[0]!.payload.payload.shptId).toBe(fromUuid('shpt', seeded.shptId))
