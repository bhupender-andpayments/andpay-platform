...
export async function ingestOpsDeviceInventory(
  ...
  const ran = await db.$transaction(async (tx: Tx) => {
    ...
    return onceWithin(tx, CONSUMER, instanceKey(args.clientKey, OPERATION), async () => {
      ...
      // flagDuplicates only, and deliberately NO flagSimNoDuplicates: the
      // Workflow A frozen rule (12 Aug 2026 walkthrough) grants Device ID
      // presence plus the duplicate-serial check as the ONLY validations on this
      // door, so a duplicate ICCID here is stored as sent and never reaches a
      // queue. Suppression by not CHECKING, not by checking and discarding.
      //
      // The inventory-ownership branch instead passed
      // persistDuplicateExceptions: false, keeping detection and dropping the
      // write. Ruled 13 Aug 2026 in favour of the frozen rule, so that is not
      // carried. Its underlying point about the SERIAL duplicate survives and is
      // recorded at the switch in intake.ts: a repeat serial still writes an
      // intake_exception whose only action re-runs the ingest and no-ops.
      ...
      // file ends up, alongside the duplicate-serial rows the ingest itself
      // flags. One exception row per (row, error code): a row can fail more than
      // one check at once (e.g. missing both Device ID and SIM No).
      for (const bad of parsed.invalidRows) {
