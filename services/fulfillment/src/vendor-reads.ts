...
//
// batch.status was dropped by the 2026-08-10 ruling (derive a batch's state
// from its children, never store a second copy). That commit corrected the ops
// reader and MISSED this one, so this query selected a column that no longer
// exists and the route threw for every vendor. The value it had shown was the
// write-once 'BORN' every batch carried for its whole life, so nothing is lost:
// openEntries against unitCount is the progress the vendor actually needs, and
// it is derived, so it cannot go stale.
export async function readVendorWorkQueue(db: FulfillmentDb, scopeVndrWire: string): Promise<WorkQueueRow[]> {
  ...
  return db.$transaction(async (tx) => {
    ...
    const rows = await tx.$queryRaw<
      { btch_id: string; unit_count: number; open_entries: bigint; created_at: Date }[]
    >`
      SELECT b.id::text AS btch_id, b.unit_count,
      ...
      GROUP BY b.id, b.unit_count, b.created_at
