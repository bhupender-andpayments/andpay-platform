...
-- is: connect as fulfillment_app, then `SET LOCAL ROLE <work role>` for the work at
...
-- band and rewires FULFILLMENT_DATABASE_URL. Until then the platform keeps
-- connecting as `andpay` exactly as before.
--
-- Membership is EXACTLY this context's work roles and nothing else. That is the
-- C4 boundary expressed in the database: fulfillment_app cannot enter another
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT FROM pg_roles WHERE rolname = 'fulfillment_app') THEN
    CREATE ROLE fulfillment_app LOGIN NOSUPERUSER NOBYPASSRLS NOINHERIT NOCREATEDB NOCREATEROLE;
  END IF;
END $$;
...
GRANT fulfillment_write, fulfillment_read, fulfillment_ops_read, fulfillment_relay, fulfillment_engine, fulfillment_vendor_read TO fulfillment_app;
