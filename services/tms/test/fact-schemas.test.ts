...
describe('TMS_FACT_SCHEMAS conform to the real emitted payloads (D120)', () => {
  ...
  it('Task 4: fct.tms.assignment.v1 declares branchCode as OPTIONAL (D120 FULL-compat, no v2)', () => {
    const schema = requireSchema(TMS_ASSIGNMENT_TOPIC)
    expect(schema.properties?.branchCode, 'schema must declare branchCode').toBeDefined()
    // optional: NOT in required, so a pre-extension payload still validates (backward compat)
    expect(schema.required ?? []).not.toContain('branchCode')
    const preExtension: AssignmentFactPayload = {
      asgnId: 'asgn_1', mrchId: 'mrch_1', progId: 'prog_1', tnntId: 'tnnt_1',
      merchantDisplayName: 'Acme', merchantLegalName: 'Acme Pvt Ltd', merchantMcc: '5814',
      bankReferenceCode: 'HDFC', bankDisplayName: 'HDFC Bank', shipToAddress: '221B Baker Street',
    ...
    // a PRE-extension payload (no branchCode) still conforms (backward-compat)
    expect(conforms(schema, preExtension)).toBe(true)
    // a NEW payload carrying it also conforms, typed string (forward-compat)
    const extended: AssignmentFactPayload = { ...preExtension, branchCode: 'BR-001' }
    expect(conforms(schema, extended)).toBe(true)
  })
