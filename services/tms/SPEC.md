...
TMS-thin is the Terminal Management context built to the minimal demand-side slice the soundbox needs (D63, D102 ownership locked; aggregate and intake per D116). It owns the bank request-file and damage-file ingest, the `asgn_` assignment aggregate (the BRD Dispatch ID), QR/VPA value custody (D102/D117), the damage/replacement workflow, and activation orchestration through the TMS device port. It depends downward only and calls no product (C1, C2); it emits facts and Fulfillment consumes them, TMS never calls Fulfillment (C5, O1). It references merchant_id and holds no identity or KYC (I1, K2, K3). It holds NO money (no ledger, no M4, no S9).
...
6. Damage and replacement (D116). A damage-file row, VPA-plus-bank matched to an original `asgn_`, creates a NEW `asgn_` with `replacement_of` set, `billable=false`, `damage_reason` from the configurable master, `bank_remarks`, `case_status=Open`, all on the replacement (no separate case aggregate). Raw: the replacement row and its reference.
