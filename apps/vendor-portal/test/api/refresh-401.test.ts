...
describe('401 interceptor', () => {
  ...
  it('a 401 triggers ONE refresh, swaps the token, retries once, succeeds', async () => {
    ...
    const { calls } = seqFetch([
      { status: 401 },                              // original vendor call
      { status: 200, body: { accessToken: 'fresh' } }, // refresh
      { status: 200, body: { rows: [] } },          // retry
    ])
    const onSessionLost = vi.fn()
    const c = createApiClient({ vendorBase: 'http://vendor', authBase: 'http://auth', onSessionLost })
    const out = await c.request<{ rows: unknown[] }>({ method: 'GET', path: '/vendor/work-queue' })
  ...
  it('a second 401 after refresh routes to login and throws', async () => {
    ...
    const onSessionLost = vi.fn()
    const c = createApiClient({ vendorBase: 'http://vendor', authBase: 'http://auth', onSessionLost })
    await expect(c.request({ method: 'GET', path: '/vendor/work-queue' })).rejects.toBeTruthy()
