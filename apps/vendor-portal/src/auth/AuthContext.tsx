...
// LeanClaim.psr) carries no top-level `role` field; class-7 (vendor
// operator) logins mint psr as `role:<name>`. Strip that prefix when
// present; otherwise fall back to the raw psr string so an unexpected
// future format still shows something instead of silently vanishing.
...
export interface AuthContextValue {
  ...
  // Exposed so feature pages (work-queue, history, return) can call the
  // typed endpoint functions under the same interceptor pipeline (401
  // refresh) the login/logout calls already use, rather than each page
  // constructing its own client with divergent deps.
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  // An unrecoverable session loss (the single refresh attempt exhausted):
  // drop the display principal so the app falls back to the login page.
  const onSessionLost = useCallback(() => { setPrincipal(null) }, [])

  const client = useMemo(() => createApiClient({
    vendorBase: (import.meta.env.VITE_VENDOR_BASE as string | undefined) ?? 'http://localhost:3010',
    authBase: (import.meta.env.VITE_VENDOR_AUTH_BASE as string | undefined) ?? 'http://localhost:3011',
    onSessionLost,
  }), [onSessionLost])
