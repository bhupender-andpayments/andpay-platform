// The 6d source-token-bucket seam (mirrors apps/auth-edge/src/throttle.ts
// byte-for-byte in shape). Defined here, in the scaffold task, so
// `VendorAuthEdgeDeps` has its final shape before the login/refresh/logout/
// enroll controllers (Tasks 9 to 12) build against it.
//
// `take` returns true when the caller may proceed and false when the source
// key (a per-IP or per-handle key) is over budget. It never throws; a
// throttled caller is a normal false, not an exception, so a controller
// decides the 429 mapping itself.
export interface ThrottlePort {
  take(sourceKey: string): Promise<boolean>
}

// The permissive default: every take() succeeds, so nothing is throttled.
// Retained as the test-helper default and as a documented no-throttle escape
// hatch; the real env builder wires InMemoryTokenBucket, so the deployed edge
...
// The 6d brute-force control: a per-source token bucket. The key is ALWAYS
// the request SOURCE (the origin IP, via sourceKey(req)), NEVER the
// credential/principal, so a third party can never lock out a victim by
...
// recovers, which is what a future test can use to force a hard 429
// boundary.
//
// This is app runtime code (not a workflow script), so Date.now() is the
// clock: a coarse-grained abuse control.
