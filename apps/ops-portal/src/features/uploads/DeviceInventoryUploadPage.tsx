...
// (invalidRows, rowNo + which field(s) were missing) by the edge and is NEVER
// ingested, without failing the whole file. A malformed row lands in the
// intake exceptions queue (/queues/intake) so it is not lost the moment this
// screen is left. A DUPLICATE row does NOT (2026-08-13 same-day follow-up,
// overturning the ruling just above: see
// docs/escalations/duplicate_rows_not_quarantined.md) - it has nothing an
// operator can correct, so it is named on THIS screen and nowhere else.
                ...
                <div className="space-y-3">
                  ...
                  {(result.flaggedRows ?? []).length > 0 && (
                    <div className="space-y-1">
                      ...
                      {/* Was "Each flagged row is also recorded in Queues,
                          under Intake exceptions." (2026-08-13: no longer
                          true, and never should have been - a duplicate has
                          nothing an operator can correct, so it is named
                          HERE and nowhere else. See
                          docs/escalations/duplicate_rows_not_quarantined.md). */}
                      <p className="text-xs text-muted-foreground">These rows were skipped. No action is needed.</p>
                    </div>
                  )}
