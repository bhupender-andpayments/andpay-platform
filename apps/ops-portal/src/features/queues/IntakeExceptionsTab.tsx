...
export function IntakeExceptionsTab() {
  ...
  const columns: GridColumn<IntakeExceptionView>[] = [
    ...
    // D-15 (T1.1) asks that a queued record show WHAT IT COLLIDED WITH, so an
    // operator can tell a vendor correction from a genuine second parcel without
    // leaving the screen. Only some reason codes carry a detail, so this is a
    // dash for the rest rather than pretending every row has an answer. The AWB
    // is the half an operator recognises; the shipment id rides as the precise
    // reference underneath it.
    {
      key: 'collidedWith',
      header: 'Already shipped as',
      sortValue: (r) => r.detail?.existingAwb ?? '',
      cell: (r) =>
        r.detail?.existingAwb === undefined || r.detail.existingAwb === null ? (
          ...
          <span className="flex flex-col">
            ...
            {r.detail.existingShptId !== null && (
              <span className="text-[11px] text-muted-foreground">{shortId(r.detail.existingShptId)}</span>
            )}
          </span>
        ),
    },
  ...
  return (
    <div className="space-y-4">
      <QueueTable
        ...
        searchText={(r) =>
          `${r.rowRef} ${r.fileId} ${vendorNames.get(r.vndrId) ?? r.vndrId} ${r.reasonCode} ${r.detail?.existingAwb ?? ''}`
        }
        vendorOf={(r) => r.vndrId}
      />
