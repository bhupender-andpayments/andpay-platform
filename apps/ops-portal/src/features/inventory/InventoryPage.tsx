...
export function InventoryPage() {
  ...
  // Two moods. 'initial' owns the skeletons; 'refresh' paints the grid's
  // "Updating..." pill for a re-read the page triggers itself (a write this
  // page just made, e.g. an upload committing). There used to be a third,
  // 'quiet', for a background poll; the poll was removed (18 Aug 2026, at the
  // user's explicit correction that a page must not keep re-reading its own
  // endpoint on a timer), and this page never grew its own Refresh button
  // (removed 2026-08-13: a browser reload already does this), so nothing
  // calls 'quiet' any more.
