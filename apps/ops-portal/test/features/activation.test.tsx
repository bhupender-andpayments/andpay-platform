...
// Task 12: the per-batch "Batches ready to send to CWD" card moved to the
// batch page's NextStepCard (Task 8). This page is the cross-batch worklist
// only, and says so.
describe('ActivationPage: slimmed to the cross-batch worklist', () => {
  beforeEach(() => {
    clearAccessToken()
    setAccessToken('tok-1')
    vi.unstubAllGlobals()
  ...
  it('has no "Batches ready to send to CWD" card any more', async () => {
    vi.stubGlobal(
      'fetch',
      vi.fn(async () =>
        jsonResponse({
  ...
  it('points batch-scoped CWD work at the batch page', async () => {
    vi.stubGlobal('fetch', vi.fn(async () => jsonResponse({ rows: [], watermark: { asOf: null, perTopic: {} } })))
    renderActivationPage()

    await screen.findByText(/nothing is awaiting activation/i)
    expect(
      screen.getByText(
        'Per-batch CWD dispatch lives on each batch page now. This list is the cross-batch worklist.',
      ),
    ).toBeTruthy()
  })
})
