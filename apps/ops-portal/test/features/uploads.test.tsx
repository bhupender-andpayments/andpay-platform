...
const DEVICE_INVENTORY_PREVIEW = {
  rows: [
    ...
    // missing_device_id, NOT missing_sim_no: the Workflow A frozen rule (12 Aug
    // 2026 walkthrough, TA.1) left Device ID presence as the only row check, and
    // this fixture stands in for a server response. A mocked fixture asserting an
    // error code the server can no longer emit is a test that passes while the
    // contract underneath it has moved.
    { rowNo: 4, deviceId: '', simNo: '', deviceQr: '', errors: ['missing_device_id'], alreadyInStock: false, simAlreadyUsed: false, duplicateInFile: false },
  ],
  totalRows: 3,
  willAdd: 1,
  willFlag: 1,
...
describe('uploads', () => {
  ...
  it('device inventory upload: a preview that fails structurally also disables Upload', async () => {
    vi.stubGlobal(
      ...
      vi.fn(async (url: string) => {
        if (url.includes('/ops/vendors')) return jsonResponse(MANUFACTURERS)
        if (url.includes('/ops/uploads/device-inventory/preview')) {
          return jsonResponse({ code: 'invalid', message: 'invalid request', reasons: [{ code: 'missing_required_column', column: 'Device ID' }] }, 400)
        }
        return jsonResponse({})
      }),
    )
  ...
  it('device inventory upload: a mixed 5-new/5-duplicate/2-needs-fixing file uploads, and only the rows that need fixing point at Queues', async () => {
    ...
    expect(screen.getByText(/these rows were skipped\. each one is also recorded in queues/i)).toBeTruthy()
