...
//
// BRD 5.3 FR-03 plus Annexure A. Two renderers consume this: the ops portal draws a
// proof in the browser, and services/fulfillment draws the artifact it STORES
// against each Dispatch ID. Those two must agree to the millimetre, or the proof an
// operator signs off is not the file a print vendor receives.
//
// It is a shared package for that reason alone. The numbers came off the bank's own
// approved output and cannot be re-derived from anything in the repo, so a second
// copy would be a second thing to keep correct with no way to tell when it drifted.
// Same rule, one home, exactly as @andpay/bank-qr.
//
